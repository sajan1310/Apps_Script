...
function getUnitsData() {
  return getCachedListResponse(MASTER_DATA_CACHE_KEYS.UNITS, () => {
    try {
...
/**
 * Propagates a Unit rename to every sheet holding a de-normalized copy of
 * the unit name, mirroring module_vendors.js's _renameVendorEverywhere.
 *
 * Without this, a rename leaves Items Master's Base/Purchase Unit columns
 * pointing at a name no longer in Unit Master — _lookupUnit() then can't
 * find it and silently falls back to factorToBase: 1 (today's implicit
 * "unknown unit" behavior), so the very next PO/Bill/Return/Wastage/Issue
 * line for that item (auto-filled from the now-stale Purchase Unit, see
 * App.Utils.applyDefaultPurchaseUnit) gets converted to the item's Base
 * Unit using the WRONG factor — e.g. "2 Gross" silently becomes a stored
 * Base Qty of 2 instead of 288 — with no error shown anywhere.
 *
 * @param {string} oldName
 * @param {string} newName
 * @private
 */
function _renameUnitEverywhere(oldName, newName) {
  ...
  // Historical ledger rows: BASE_QTY was already computed and frozen at
  // save time using whatever factor was in effect then, so these renames
  // are cosmetic consistency only (never retroactively correct math) —
  // but leaving them stale would show the same real unit under two
  // different names depending on when the line was entered.
  renameInColumn(APP_CONFIG.SHEETS.PO, PO_COL.UNIT, APP_CONFIG.PO_SETTINGS.DATA_START_ROW);
  renameInColumn(APP_CONFIG.SHEETS.BILL, BILL_COL.UNIT, 2);
  renameInColumn(APP_CONFIG.SHEETS.RETURN, RETURN_COL.UNIT, 2);
  renameInColumn(APP_CONFIG.SHEETS.WASTAGE, WASTAGE_COL.UNIT, 2);
  renameInColumn(APP_CONFIG.SHEETS.ISSUE, ISSUE_COL.UNIT, 2);

  // Items Master: THIS is the reference that actually matters going
  // forward — Base Unit drives every future conversion for the item, and
  // Purchase Unit is what auto-fills a new PO/Bill line's Unit field.
  let itemsSheet;
  try {
  ...
  if (itemsSheet) {
    ...
    if (lastRow >= 2) {
      const range = itemsSheet.getRange(2, ITEMS_COL.BASE_UNIT, lastRow - 1, ITEMS_COL.PURCHASE_UNIT - ITEMS_COL.BASE_UNIT + 1);
      const values = range.getValues();
      let changed = false;
      for (let i = 0; i < values.length; i++) {
  ...
  // Items Master's Base/Purchase Unit columns were potentially rewritten above.
  ...
  try {
    ...
    if (isEdit) {
      ...
      if (newName !== originalName) {
        _renameUnitEverywhere(originalName, newName);
      }
    } else {
    ...
    SpreadsheetApp.flush();
    invalidateListCache(MASTER_DATA_CACHE_KEYS.UNITS);
