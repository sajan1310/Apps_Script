  ...
  try {
    ...
    Object.values(vendorMap).forEach(v => {
      ...
      if (!existing) {
        ...
        vSheet.getRange(existing.row, VENDORS_COL.CONTACT).setValue(sanitizeString(v.contact, 'contact'));
    ...
    Object.values(itemMap).forEach(item => {
      ...
      if (item.narration) {
        const narrationCell = iSheet.getRange(matchRow, ITEMS_COL.NARRATION);
        if (!String(narrationCell.getValue() || '').trim()) {
          narrationCell.setValue(sanitizeString(item.narration, 'narration'));
          changed = true;
        }
      }
      ...
      if (lastCol >= ITEMS_COL.VENDOR_DATA) {
        const vendorRange = iSheet.getRange(matchRow, ITEMS_COL.VENDOR_DATA, 1, lastCol - ITEMS_COL.VENDOR_DATA + 1).getValues()[0];
        for (let v = 0; v + 1 < vendorRange.length; v += 2) {
          const vName = String(vendorRange[v] || '').trim();
          const vRateRaw = vendorRange[v + 1];
