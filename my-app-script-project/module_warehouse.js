...
// and folded into the matching bucket's Produced Qty.

const WAREHOUSE_POOL_OPENING_LOCK_TIMEOUT_MS = 15000;
...
function saveWarehousePoolOpening(formData) {
  ...
  if (!lock.tryLock(WAREHOUSE_POOL_OPENING_LOCK_TIMEOUT_MS)) {
  ...
  try {
    ...
    if (qty <= 0) {
      return buildResponse(false, null, 'Opening Quantity must be greater than zero.');
    }
    ...
    sheet.appendRow([process.outputItemName, process.processId, productTag, color, qty, dateStr, remarks]);
 ...
 * traceable, mirroring adjustStockManually() for raw-material Stock.
...
function adjustWarehousePoolManually(outputItemName, processId, productTag, color, newAvailableQty, reason) {
  ...
  try {
    ...
    if (isNaN(newQty) || newQty < 0) {
      return buildResponse(false, null, 'Corrected quantity must be a valid non-negative number.');
    }
    ...
    if (delta === 0) {
      return buildResponse(false, null, 'New quantity is the same as the current value — nothing to adjust.');
    }
    ...
    const dateStr = toSafeDateString(new Date());
    openingSheet.appendRow([itemName, procId, tag, colorVal, delta, dateStr, `Correction: ${reasonText}`]);

    recalculateWarehousePool();
    SpreadsheetApp.flush();

    const recordId = `${itemName}|${tag}|${colorVal}`;
  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      records.push({
        ...
        outputItemName: idParts[0],
        productTag: idParts[1],
        color: idParts[2],
...
/**
 * ONE-TIME SETUP — run manually from the Apps Script editor when adopting
 * the Process+Product merge / Warehouse Pool model on a spreadsheet that
 * already has data under the old Production/BOM layout.
 *
 * Renames the existing 'Production' and 'BOM' sheets to
 * 'Production_Archive_<YYYYMMDD>' / 'BOM_Archive_<YYYYMMDD>' (preserving
 * all historical data for reference/reporting) and creates fresh, empty
 * 'Production' and 'BOM' sheets with the new headers so the app can start
 * clean under the new model.
 *
 * Safe to run only once — if 'Production' or 'BOM' have already been
 * archived (no sheet by that name exists), this is a no-op for that sheet.
 */
function archiveLegacyProductionAndBOM() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const stamp = Utilities.formatDate(new Date(), Session.getScriptTimeZone() || 'Etc/UTC', 'yyyyMMdd');

  const results = [];

  [APP_CONFIG.SHEETS.PRODUCTION, APP_CONFIG.SHEETS.BOM, APP_CONFIG.SHEETS.BOM_COSTS].forEach(sheetName => {
    const sheet = ss.getSheetByName(sheetName);
    if (!sheet) {
      results.push(`"${sheetName}" not found — nothing to archive.`);
      return;
    }
    const archiveName = `${sheetName}_Archive_${stamp}`;
    if (ss.getSheetByName(archiveName)) {
      results.push(`"${archiveName}" already exists — skipped.`);
      return;
    }
    sheet.setName(archiveName);
    results.push(`Archived "${sheetName}" -> "${archiveName}".`);
  });
  ...
  SpreadsheetApp.flush();
  console.log('[archiveLegacyProductionAndBOM] ' + results.join(' '));
  return buildResponse(true, { results: results }, 'Legacy Production/BOM data archived; fresh sheets created.');
}
