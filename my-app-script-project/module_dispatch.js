...
function saveDispatch(formData) {
  ...
  try {
    ...
    if (isEdit) {
      ...
      const currentProductId = String(sheet.getRange(targetRow, DISPATCH_COL.PRODUCT_ID).getValue()).trim();
      ...
      originalQty = Number(sheet.getRange(targetRow, DISPATCH_COL.QTY).getValue()) || 0;
    }
    ...
    const dispatchNumber = isEdit
      ? String(sheet.getRange(targetRow, DISPATCH_COL.DISPATCH_NUMBER).getValue()).trim()
...
function deleteDispatch(rowIdx, expectedDispatchNumber, expectedQty) {
  ...
  try {
    ...
    const dispatchNumber = String(sheet.getRange(targetRow, DISPATCH_COL.DISPATCH_NUMBER).getValue()).trim();
    const qty = Number(sheet.getRange(targetRow, DISPATCH_COL.QTY).getValue()) || 0;
