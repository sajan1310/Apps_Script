...
function deleteClientsBulk(clientNames) {
  ...
  try {
    ...
    const targetSet = new Set(
      (clientNames || []).map(n => String(n || '').trim().toLowerCase()).filter(Boolean)
    );
    if (targetSet.size === 0) {
    ...
    const lastRow = sheet.getLastRow();
    if (lastRow < 2) {
      return buildResponse(true, null, 'No clients to delete.');
    }
    ...
    const msg = `Deleted ${rowsDeleted} client(s) from master.`;
...
function saveClientOrder(formData) {
  ...
  try {
    ...
    let dateVal = formData.orderDate;
    ...
    // Carry forward "Production Pushed" flags from existing rows (by Product ID)
    const pushedMap = {};
    if (isEdit) {
      const lastRow = sheet.getLastRow();
      if (lastRow >= 2) {
        ...
        existingData.forEach(row => {
          if (String(row[CLIENT_ORDERS_COL.ORDER_NUMBER - 1] || '').trim().toLowerCase() === orderNumber.toLowerCase()) {
            ...
            if (pid && pushed) pushedMap[pid] = true;
          }
        });
      }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      cleanLines.push({
        ...
        productionPushed: !!pushedMap[productId.toLowerCase()],
...
function _pushOrderLinesToProduction(orderNumber, clientName, lines) {
  ...
  lines.forEach(line => {
    ...
    row[PRODUCTION_COL.DATE - 1] = dateStr;
...
function deleteClientOrdersBulk(orderNumbers) {
  ...
  try {
    ...
    return buildResponse(true, { deletedIds, skipped: Array.from(inUseSet) }, msg);
  } catch (error) {
