...
function getBillData() {
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // Create bill entry if first time seeing this bill number
      if (!billMap[billNum]) {
      ...
      if (rowPoNum && !billMap[billNum].poNumbers.includes(rowPoNum)) {
        billMap[billNum].poNumbers.push(rowPoNum);
      }
      ...
      billMap[billNum].totalQty += qty;
      billMap[billNum].totalAmount += lineTotal;
    }
 ...
 *
 * @returns {Object} map keyed by "poNumber|name|size|narration" (lowercased) -> baseQty
 */
...
function saveBill(formData) {
  ...
  try {
    ...
    if (_billNumberExists(sheet, startRow, billNumber, isEdit ? oldBillNum : null)) {
      return buildResponse(
        false,
        null,
        isEdit
          ? `The bill number "${billNumber}" already exists in another record. Edit aborted.`
          : `Bill number "${billNumber}" already exists. Please use a unique bill number.`
      );
    }

    let firstMatchRow = -1;
    if (isEdit) {
      firstMatchRow = _findFirstBillRow(sheet, startRow, oldBillNum);
      if (firstMatchRow === -1) {
        throw new Error(
          `Original Bill #${oldBillNum} not found in sheet. Edit aborted to prevent data corruption.`
        );
      }
    }
    ...
    // ─────────────────────────────────────────────────────────────────
    const billDate = toSafeDateString(formData.billDate);
    if (!billDate) {
      return buildResponse(
        false,
        null,
    ...
    // Convert DD/MM/YYYY to native Date for sheet storage
    const billDateParts = billDate.split('/');
    const billDateNative = new Date(
      parseInt(billDateParts[2], 10),
      parseInt(billDateParts[1], 10) - 1,
      parseInt(billDateParts[0], 10)
    );
    ...
    const newRows = items.map(function(item) {
      ...
      const gstRatePct = _validateGstRate(item.gst || DEFAULT_GST_RATE_PCT);
    ...
    // ─────────────────────────────────────────────────────────────────
    if (isEdit) {
      _rewriteBillsWithoutMatchingRows(sheet, startRow, oldBillNum);
 ...
 * 
 * Deletes all rows for a given bill number
 * 
 * Operations:
 * 1. Acquire document lock
 * 2. Validate bill number
 * 3. Remove all rows from Bill sheet matching bill number
 * 4. Flush changes
 * 
 * Safety:
 * - Document lock prevents concurrent deletes
 * - Validates bill number before deletion
...
function deleteBill(billNumber) {
  ...
  try {
    ...
    // Delete all rows matching bill number using efficient batch deletion
    deleteRowsById(
      validBillNumber,
      sheet,
      APP_CONFIG.BILL_SETTINGS.DATA_START_ROW,
      BILL_COL.BILL_NUMBER
    );
...
/**
 * deleteBillsBulk(billNumbers)
 *
 * Deletes all rows for multiple bill numbers in a single batch operation.
 * Mirrors deleteBill() but rewrites the sheet once instead of once per bill.
 *
 * @param {Array<string|number>} billNumbers - Bill numbers to delete
 * @returns {Object} API response
 */
function deleteBillsBulk(billNumbers) {
  const lock = LockService.getDocumentLock();
  ...
  try {
    const targetSet = new Set(
      (billNumbers || []).map(b => String(b).trim()).filter(Boolean)
    );
    ...
    const msg = `Deleted ${targetSet.size} bill(s) (${rowsDeleted} row(s) removed).`;
...
/**
 * Checks if a bill number already exists in the sheet.
 * Can exclude a specific bill number (useful during edits).
 * 
 * @param {Sheet} sheet - Bill ledger sheet
 * @param {number} startRow - Start row
 * @param {string} billNumber - Bill number to check
 * @param {string} [excludeBillNumber] - Bill number to exclude from check
 * @returns {boolean} True if duplicate exists
 * @private
 */
function _billNumberExists(sheet, startRow, billNumber, excludeBillNumber = null) {
  const lastRow = sheet.getLastRow();
  if (lastRow < startRow) return false;

  const values = sheet
    .getRange(startRow, BILL_COL.BILL_NUMBER, lastRow - startRow + 1, 1)
    .getValues();
  
  const target = String(billNumber).trim().toLowerCase();
  const exclude = excludeBillNumber ? String(excludeBillNumber).trim().toLowerCase() : null;

  for (let i = 0; i < values.length; i++) {
    const val = String(values[i][0] || '').trim().toLowerCase();
    if (val === target) {
      if (exclude && val === exclude) {
        continue;
      }
      return true;
    }
...
/**
 * Finds the first row number matching a bill number.
 * 
 * @param {Sheet} sheet - Bill ledger sheet
 * @param {number} startRow - Start row
 ...
 * @private
 */
function _findFirstBillRow(sheet, startRow, billNumber) {
  const lastRow = sheet.getLastRow();
...
/**
 * Removes all rows matching a specific bill number from the sheet.
 ...
 * @private
 */
function _rewriteBillsWithoutMatchingRows(sheet, startRow, billNumber) {
  const lastRow = sheet.getLastRow();
  const lastCol = sheet.getLastColumn();
  ...
  const matchIndex = BILL_COL.BILL_NUMBER - 1;
  const targetStr = String(billNumber).trim().toLowerCase();
