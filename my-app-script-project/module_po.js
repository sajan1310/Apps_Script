...
function _getPoDataRangeValues(sheet, startRow) {
  ...
  const numCols = PO_COL.SUPPLIER_REMARKS - PO_COL.PO_NUMBER + 1;
  return sheet
 ...
 *   items: [
 *     { name, narration, size, qty, unit, price, total }
 *   ],
 *   grandTotal: 12500,
 *   totalQty: 100
 * }
...
function suggestPoAllocations(vendor, items, billDate) {
  try {
    ...
    const billDateStr = toSafeDateString(billDate);
    if (billDateStr) {
      // toSafeDateString returns DD/MM/YYYY; rebuild as a comparable Date.
    ...
    vendorPOs.forEach(function(po) {
      (po.items || []).forEach(function(poItem) {
        const key = po.poNumber + '|' +
          String(poItem.name).trim().toLowerCase() + '|' +
          String(poItem.size).trim().toLowerCase() + '|' +
          String(poItem.narration).trim().toLowerCase();
