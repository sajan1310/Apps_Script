...
function doGet(e) {
  try {
    ...
    // ───────────────────────────────────────────────────────��─────────
    // STEP 2: Load and Configure HTML Template
    // ─────────────────────────────────────────────────────────────────
    ...
    template.routeData = e ? (e.parameter || {}) : {};
    template.isProduction = PRODUCTION_MODE;

    // ─────────────────────────────────────────────────────────────────
    // STEP 3: Render Template and Configure Output
    // ─────────────────────────────────────────────────────────────────
    const output = template.evaluate();

    // Set application title
    output.setTitle(appTitle);
    ...
    console.log(`[doGet] Successfully rendered ${appTitle}`);
...
function include(filename) {
  ...
  try {
    ...
    if (rawContent.indexOf('<?') !== -1) {
      ...
      console.log(`[include] Successfully evaluated component with scriptlets: ${filename}`);
      return fileContent;
    } else {
      console.log(`[include] Successfully loaded static/script component: ${filename}`);
      return rawContent;
    }
  } catch (error) {
