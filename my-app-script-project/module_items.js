 ...
 * Blank/undefined defaults to 0.
 *
 * @param {*} value - Raw initial stock input
 * @returns {number} Validated non-negative initial stock
 * @throws {Error} If a non-empty value is not a valid non-negative number
 */
...
function deleteItemsBulk(items) {
  ...
  try {
    ...
    // Map the items to delete to helper sets for faster lookup
    const targetSet = new Set((items || []).map(item => {
    ...
    (items || []).forEach(item => {
      ...
      if (name) stockKeysToCheck.set(name.toLowerCase() + '|' + size.toLowerCase(), { name, size });
    });
    ...
    const msg = `Successfully deleted ${rowsDeleted} item(s) from master.`;
    logAction('BULK_DELETE', APP_CONFIG.SHEETS.ITEMS, 'multiple', msg, 'SUCCESS');
    return buildResponse(true, { deletedItems }, msg);
  } catch (error) {
