...
function recalculateStock() {
  ...
  try {
    ...
    SpreadsheetApp.flush();
    console.log('[recalculateStock] Successfully updated stock values.');
  } catch (error) {
...
function adjustStockManually(itemName, size, newCurrentStock, reason) {
  ...
  try {
    ...
    if (isNaN(newStockVal) || newStockVal < 0) {
      return buildResponse(false, null, 'Corrected stock must be a valid non-negative number.');
    }
  ...
  try {
    ...
    if (action === 'ensure') {
      ...
      if (oldRow === -1) {
        ...
        if (newRow === -1) {
          sheet.appendRow([newName, newSize, 0, 0, 0]);
        }
      } else if (newRow === -1) {
