...
function getBOMProductionData() {
  try {
    ...
    const products = Object.values(productMap);
    // Match getBOMData's manual display order (drag-and-drop reorderable on
    // the Products tab) — this list feeds the Production/Warehouse Opening
    // Product dropdowns, which should reflect the same order the user set.
    products.sort((a, b) => a.sequence - b.sequence);
...
/**
 * @private
 * Returns the subset of productIds (matched case-insensitively) that are
 * currently referenced elsewhere and therefore unsafe to delete:
 *  - a Production lot tagged with the product (PRODUCTION_COL.PRODUCT_ID —
 *    only set on final-stage lots)
 *  - a Client Order / PI-Estimate line (CLIENT_ORDERS_COL.PRODUCT_ID)
 *  - a Dispatch record (DISPATCH_COL.PRODUCT_ID)
 * Since Production's PRODUCT_ID is only ever set on final-stage lots, a
 * Product with open Client Orders or Dispatch history but no tagged
 * final-stage lot YET would sail past a Production-only check and get
 * deleted, leaving those rows pointing at a Product ID the BOM sheet no
 * longer has any data for (same class of gap as module_process.js's
 * deleteProcess, which had the same Production-only blind spot for BOM
 * references). Shared by deleteBOM and deleteBOMsBulk.
 * @param {string[]} productIds
 * @returns {Set<string>} lower-cased product IDs found in use
 */
function _getProductIdsInUse(productIds) {
  const requested = new Set((productIds || []).map(id => String(id || '').trim().toLowerCase()).filter(Boolean));
  const inUse = new Set();
  if (requested.size === 0) return inUse;

  const collectRefs = (sheetName, col) => {
    let sheet;
    try {
    ...
    const lastRow = sheet.getLastRow();
    if (lastRow < 2) return;
    const refs = sheet.getRange(2, col, lastRow - 1, 1).getValues();
    refs.forEach(row => {
  ...
  collectRefs(APP_CONFIG.SHEETS.PRODUCTION, PRODUCTION_COL.PRODUCT_ID);
  collectRefs(APP_CONFIG.SHEETS.CLIENT_ORDERS, CLIENT_ORDERS_COL.PRODUCT_ID);
  collectRefs(APP_CONFIG.SHEETS.DISPATCH, DISPATCH_COL.PRODUCT_ID);
 ...
 * Before deleting, checks if the Product ID is in use by Production,
 * Client Orders, or Dispatch (see _getProductIdsInUse).
 * Requires a valid BOM access token (see verifyBOMAccess).
...
function deleteBOM(productId, token) {
  ...
  try {
    ...
    if (_getProductIdsInUse([prodIdClean]).has(prodIdClean.toLowerCase())) {
      return buildResponse(
        false,
        null,
        `Cannot delete BOM: Product ID "${prodIdClean}" is already referenced by Production lots, a Client Order, or Dispatch. Clear those first.`
      );
    }
 ...
 * Any product currently referenced by Production, a Client Order, or
 * Dispatch is skipped (and reported back) rather than failing the entire
 * batch (see _getProductIdsInUse).
 * Requires a valid BOM access token (see verifyBOMAccess).
...
function deleteBOMsBulk(productIds, token) {
  ...
  try {
    ...
    const inUseLower = _getProductIdsInUse(requested);
    ...
    if (inUseSet.size > 0) {
      msg += ` Skipped ${inUseSet.size} BOM(s) still in use by Production, a Client Order, or Dispatch: ${Array.from(inUseSet).join(', ')}.`;
    }
