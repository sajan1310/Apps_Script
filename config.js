...
const PROCESS_COL = Object.freeze({
  ...
  PRIMARY_COLOR_AXIS: 10, // Color Axis label (see PROCESS_COMPONENTS_COL.COLOR_AXIS) whose checked "Colors to Produce" rows determine a lot's total quantity, when this process has 2+ independent color axes. Every process that HAS an axis has one: left blank, saveProcess/saveProduction resolve and store the process's first axis in recipe order (see _defaultPrimaryColorAxisLabel, module_process.js) rather than falling back to the legacy sum-every-axis behavior, which double-counts non-primary rows. Only a process with no color axes at all stays blank — that's the legacy single-list behavior (see computeColorGroupsForProcess), unchanged.
